...
addingHighResImage(unsigned int rotation)
{
   unsigned int width = 2304;
   unsigned int height = 1536;
   ...
   unsigned int bytes;
   _cam->grab(_img, &bytes);
   char* path1 = (char*)"/tmp/snapshot_";
   char* path2 = (char*)rotation;
   char* path3 = (char*)".ppm";
   char* path = strcat(path1, path2);
   path = strcat(path, path3);
   serializePPM(path, _img, _cam->getWidth(), _cam->getHeight(), 0);
   toggleMJPEG();
   cout << "snapshot saved to " << path << endl;
}
...
int main(int argc, char* argv[])
{
   ...
   viewer.registerKeyboardCallback('r', addingHighResImage(rotation));
